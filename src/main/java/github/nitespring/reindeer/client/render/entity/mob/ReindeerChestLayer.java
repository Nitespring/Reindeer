...
public class ReindeerChestLayer<T extends AbstractReindeer & GeoAnimatable, O, R extends GeoRenderState> extends GeoRenderLayer<T, O, R> {

    private static final Identifier RESOURCE = Identifier.fromNamespaceAndPath(ReindeerMod.MODID, "textures/entity/reindeer/reindeer_bags.png");


    public ReindeerChestLayer(GeoRenderer<T, O, R> renderer) {
        super(renderer);
    ...
    public void submitRenderTask(RenderPassInfo<R> renderPassInfo, SubmitNodeCollector renderTasks) {
        ...
        if(getTextureResource(renderPassInfo.renderState())!=null) {
            ...
            if (!renderPassInfo.renderState().hasGeckolibData(CustomDataTickets.IS_BABY)
                ...
                if (renderPassInfo.renderState().hasGeckolibData(CustomDataTickets.HAS_CHEST)
                        && renderPassInfo.renderState().getGeckolibData(CustomDataTickets.HAS_CHEST)) {
                    if (renderType != null)
